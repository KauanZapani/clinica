        ...
        {isAdmin && (
          <Dialog open={isCreateOpen} onOpenChange={(open) => { setIsCreateOpen(open); if (!open) resetForm(); }}>
            <DialogTrigger asChild>
              <Button data-testid="button-novo-servico"><Plus className="w-4 h-4 mr-2" /> Novo Serviço</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-muted/50">
              <tr>
                ...
                {isAdmin && <th className="p-4 font-medium">Ações</th>}
              </tr>
            </thead>
            <tbody>
              {services?.map((service) => (
                <tr key={service.id} className="border-t" data-testid={`row-service-${service.id}`}>
                  <td className="p-4">
                  ...
                  {isAdmin && (
                    <td className="p-4 flex gap-2">
                      <Button variant="outline" size="icon" onClick={() => openEdit(service)} data-testid={`button-edit-service-${service.id}`}>
                        <Edit className="w-4 h-4" />
                      </Button>
                      <Button variant="outline" size="icon" className="text-destructive hover:bg-destructive/10" onClick={() => handleDelete(service.id)} data-testid={`button-delete-service-${service.id}`}>
              ...
              {services?.length === 0 && (
                <tr>
                  <td colSpan={isAdmin ? 5 : 4} className="p-8 text-center text-muted-foreground">Nenhum serviço encontrado.</td>
                </tr>
              )}
            </tbody>
          </table>
